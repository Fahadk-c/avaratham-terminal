# Avaratham Terminal

A VS Code extension that plays a random short sound clip whenever a command in
the integrated terminal finishes with a **non-zero exit code**. Your build
breaks, your test fails, your `git push` gets rejected — and you hear about it.

## How it works

The extension listens to the stable Terminal Shell Integration API
(`vscode.window.onDidEndTerminalShellExecution`). When a command reports a
non-zero exit code, it picks a random clip from a sounds folder and plays it.

- **No repeats back-to-back** — the clip that just played is excluded from the
  next draw (as long as there is more than one clip).
- **Cooldown** — a configurable minimum gap between triggers so a burst of
  fast failures doesn't turn into a wall of noise.
- **Playback** — the system audio player (`afplay` on macOS, PowerShell on
  Windows, `ffplay`/`paplay`/`aplay` on Linux), with a webview player as
  fallback. See [Playback modes](#playback-modes) for why.

> Terminal shell integration must be active for the extension to see exit
> codes. It is enabled by default in recent VS Code versions for bash, zsh,
> fish and PowerShell.

## Running it locally

```bash
npm install
npm run compile
```

Then press <kbd>F5</kbd> in VS Code to launch the **Extension Development
Host** — a second VS Code window with the extension loaded. Open a terminal in
that window and run something that fails, e.g.:

```bash
false
```

(Use `false`, not `exit 1` — `exit` closes the terminal session before shell
integration can report the result.)

Use `npm run watch` instead of `npm run compile` if you want TypeScript to
recompile on every save while you work.

## Sounds

The repo ships with real clips in `sounds/malayalam/`. To add your own, either:

1. Drop `.mp3`, `.m4a`, `.aac`, `.wav` or `.ogg` files straight into
   `sounds/malayalam/`, or
2. Point the extension at any folder on disk via the
   `avarathamTerminal.soundsFolder` setting.

The pool re-reads the folder on every trigger, so new clips are picked up
immediately — no window reload needed.

If you ever want quick placeholder tones instead (e.g. for testing before you
have real clips), `npm run generate-placeholders` synthesizes five short sine
tones into `sounds/malayalam/` with a pure-Node script (no `ffmpeg`/`sox`
needed).

Keep clips short — 1 to 3 seconds — so they don't bleed into your next
command.

## Commands

Both are available from the Command Palette (<kbd>Cmd/Ctrl</kbd> +
<kbd>Shift</kbd> + <kbd>P</kbd>):

| Command | What it does |
| --- | --- |
| `Avaratham Terminal: Toggle On/Off` | Flips the `enabled` setting globally |
| `Avaratham Terminal: Play a Random Test Sound` | Plays one clip immediately, so you can check audio works |

## Settings

| Setting | Type | Default | Description |
| --- | --- | --- | --- |
| `avarathamTerminal.enabled` | boolean | `true` | Turn sound-on-fail on or off. |
| `avarathamTerminal.cooldownMs` | number | `3000` | Minimum milliseconds between two triggers. Keep at or above your longest clip. |
| `avarathamTerminal.soundsFolder` | string | `""` | Absolute path to your own clips folder. Empty means use the bundled `sounds/malayalam`. |
| `avarathamTerminal.playbackMode` | string | `"auto"` | `auto`, `system`, or `webview`. See below. |
| `avarathamTerminal.ignoredExitCodes` | number[] | `[130]` | Exit codes that never trigger a sound. |
| `avarathamTerminal.ignoredCommands` | string[] | grep/rg/git diff/diff | Regex patterns; a match means no sound. |

Changing `soundsFolder` reloads the sound pool immediately — no window reload
required.

## What counts as a failure

A non-zero exit code does not always mean something broke. `grep` returns 1
when it simply found no matches, `git diff --quiet` returns 1 to say "there
are changes", and Ctrl+C returns 130. Announcing those gets old fast, so two
settings filter them out:

- `ignoredExitCodes` — defaults to `[130]`, i.e. Ctrl+C stays silent.
- `ignoredCommands` — regexes matched against the command line, defaulting to
  `grep`, `rg`, `git diff` and `diff`.

Word boundaries in the defaults keep them narrow: `grep foo` is silent, but
`grepper` and `git difftool` still make noise. An invalid regex is skipped
with a one-time warning rather than silencing everything.

## Audio formats

`.mp3`, `.m4a`, `.aac`, `.wav` and `.ogg` are picked up from the sounds folder.

`.mp3` and `.m4a`/`.aac` are the safe choices. **Avoid `.ogg` on macOS** —
CoreAudio has no Ogg Vorbis support, so `afplay` fails and the clip is silently
skipped with no error.

Keep clips at or under the `cooldownMs` value (3 seconds by default). Nothing
interrupts a clip that is already playing, so anything longer can overlap with
the next one.

## Playback modes

The obvious approach — a webview with `<audio autoplay>` — **does not work**.
Chromium's autoplay policy rejects it with `NotAllowedError: play() can only be
initiated by a user gesture`, because a Command Palette invocation or a failing
terminal command is not a click inside the webview document. So playback goes
through the OS instead:

| Platform | Player used |
| --- | --- |
| macOS | `afplay` (ships with the OS) |
| Windows | PowerShell — `Media.SoundPlayer` for `.wav`, `MediaPlayer` for `.mp3`/`.ogg` |
| Linux | first of `ffplay`, `paplay`, `play` (sox), `mpg123`, `aplay` that is installed |

If no player is found, it falls back to the webview panel, which shows an
**"Enable fail sounds"** button. One click grants that document sticky user
activation and every later clip plays normally, for as long as the tab stays
open.

`playbackMode` forces the choice: `system` never opens a panel, `webview`
always uses one, `auto` (the default) prefers the system player.

## Copyright note on audio

The clips in `sounds/malayalam/` are committed to this public repo. If you add
more, keep in mind that film dialogue, music and other commercial recordings
are generally not okay to redistribute without rights. To keep a personal clip
out of the repo entirely, point `avarathamTerminal.soundsFolder` at a folder
outside the project instead of dropping it in `sounds/malayalam/`.

## License

MIT — see [LICENSE](LICENSE).
